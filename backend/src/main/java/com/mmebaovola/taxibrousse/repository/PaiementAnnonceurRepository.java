...
public interface PaiementAnnonceurRepository extends JpaRepository<PaiementAnnonceur, Long> {
        ...
        // Total de tous les paiements d'un annonceur (sans filtre de période)
        @Query("SELECT COALESCE(SUM(p.montantPaye), 0) FROM PaiementAnnonceur p WHERE p.annonceur.id = :annonceurId")
        BigDecimal sumAllPaymentsByAnnonceur(@Param("annonceurId") Long annonceurId);
